...
export const ChatHeader = () => {
  ...
  const headerTitle = currentRoom.title || otherParticipant?.name || '알 수 없음';
  const headerImage = (otherParticipant?.profileKey && imageMap[otherParticipant.profileKey]) 
    ...
    <div>
      ...
      <header className="w-full px-4 h-12 flex flex-row shrink-0 items-center justify-between bg-Ivory">
        ...
        >
          ...
          <span className='text-body-02'>{totalUnreadCount > 0 ? totalUnreadCount : ''}</span>
        </div>
        ...
        <div className="flex items-center">
          <img src={video} alt="영상통화" className="w-6 h-6 mr-3 cursor-pointer" />
